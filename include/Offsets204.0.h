#pragma once

#include <cstdint>

namespace Offsets::V204_0
{
    constexpr uintptr_t ArgCount = 0x3505F0;
    constexpr uintptr_t GetArgFloat = 0x350648;
    constexpr uintptr_t GetArgInteger = 0x3508B9;
    constexpr uintptr_t GetArgString = 0x350888;
    constexpr uintptr_t DeleteItem = 0x34BC44;
    constexpr uintptr_t CreateCmd = 0x34BC64;
    constexpr uintptr_t RegisterHandler = 0x34BBA4;
    constexpr uintptr_t inSatellite = 0x277078;
    constexpr uintptr_t netPlayerInfoArray = 0x1C689E;
    constexpr uintptr_t activeConfigMod = 0x102708;
    constexpr uintptr_t misnExport = 0xA185A;
    constexpr uintptr_t globalHandler = 0x34B4AA;
    constexpr uintptr_t GameObjectClass_Find = 0x166E2B;
    constexpr uintptr_t SchedPlan_GoodSpot = 0x2A3CC0;
} // namespace Offsets